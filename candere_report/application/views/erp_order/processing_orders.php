...
echo '<form method="POST" name="sts_form" action="'.base_url('index.php/erp_order/processing_orders').'">';
		...
		$user_status = '';
			foreach($show_orders as $ab){
				$user_status[$ab->sequence] = $ab->status_name;
			}
		...
		echo $status_dropdown = form_multiselect('search_by_status[]', $user_status, $selected_status, 'id="search_by_status" class="SlectBox" placeholder="Select Status"');
...
<input type="hidden" name="viewtype" value="processing">
...
<?php
echo '<form name="form_processing_order_sort" id="form_processing_order_sort" method="POST" action="'.$_SERVER['PHP_SELF'].'">';
$options = array(
					'order_id'  				=> 'Order Id',
...
<form action="<?php echo base_url('index.php/erp_order/processing_orders'); ?>" enctype="multipart/form-data" method="post">
	<br /> 
	<input type = "submit" value = "Export Data" name="sbmt_processing_orders" id="sbmt_processing_orders" class="styled-button-1" style="width:150px;" />
</form>
<script>
$(document).ready(function() {
	$("#dir_toggle").click(function(){
		...
		$.cookie("sort_dir", status_id[1], { expires: 1 });
		$('#direction').val($.cookie("sort_dir")); 
		$("#form_processing_order_sort").submit();
		return false;
	});
...
if($processdata)
{
	...
	$process_status_option[''] = 'Select Status';
		foreach($process_status as $nb){
			$process_status_option[$nb->sequence] = $nb->status_name;
		}
	...
	foreach($processdata as $rj){
		...
		$order_item_id 		= $rj->order_item_id;
		...
		$vendor_dropdown = form_dropdown('vendor_id', $vendor_option, $vendor_id,'id="VendorID_'.$trProcess.'", class="tb10" style="width:150px;" onchange="return submitForm("1", "2")');
		
		//onchange="return submitForm(this.value, "order_update_'.$trProcess.'")"
		...
		//$order_currency_code = $rj->order_currency_code ;
		if($rj->order_currency_code=='USD' || $rj->mktplace_name=='Amazon.com')
		{
			$usd_total = $total ;
		}
	else
	{
		...
		$process_table .= '<tr class="'.$bgclass.' main_tr" id="'.$trProcess.'"  >
			...
			<b>Order Item Id : '.$rj->order_item_id.'</b><br>
			<img src="'.str_replace('cdn','www',$rj->product_image).'" height="120" width="120" class="img_padding">
			...
			<td class="bottom" width="580" valign="top" style="padding-top:10px;">
				<table name="product_data" width="100%">
					<tbody>
						<tr><td><b>
						SKU : </b>'.$rj->sku.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/>
						<b>CUSTOM SKU : </b>'.$rj->sku_custom.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<b>Status : <u class="red">'.$status_name.'</u></b>						
							</td></tr>
						<tr><td><b><a href="'.$product_url.'" style="color:#0066CC;text-decoration:none;" target="_blank">'.$rj->product_name.'</a></b>&nbsp;&nbsp;&nbsp;<b style="color:#0066CC;float:right;margin-right:10%;">Design No. : '.$rj->design_identifier.'</b></td>
						</tr>
						<tr>
							<td width="400">
							...
							<tbody>
								...
								if($rj->mktplace_name!='')
								{
									$process_table .='<td><b>Marketplace name : </b>'.$rj->mktplace_name.'</td>';
								}
								$process_table .='</tr>';
								$process_table .='<tr>';
								if($rj->mktplace_order_id!='')
								{
									$process_table .='<td><b>Marketplace Order id : </b>'.$rj->mktplace_order_id.'</td>';
								}
								$process_table .='</tr>';
								$process_table .='<tr>';
								if($rj->mktplace_sub_order_id!='')
								{
									$process_table .='<td><b>Marketplace Sub Order Id : </b>'.$rj->mktplace_sub_order_id.'</td>';
								}
								$process_table .='</tr>';
								...
								$process_table .='</tr>';
								$process_table .='<tr>';
								if($rj->engrave_message!='') {
									$process_table .= '<td><b>Engrave Message : </b>'.$rj->engrave_message.'</td>';
								}
								$process_table .='</tr>';
								
								$process_table .='<tr>';
								if($rj->engrave_font!='') {
									$process_table .= '<td><b>Engrave Font : </b>'.$rj->engrave_font.'</td>';
								}
								$process_table .='</tr>';
								
								
								if($rj->ring_size!='0') {
									$process_table .= '<td><b>Ring Size : </b>'.$rj->ring_size.'</td>';
								}
								...
								$process_table .= '<tr>';
								if($rj->chain_thickness) {
									$process_table .='<td width="150"><b>chain Thickness : </b>'.$rj->chain_thickness.' mm</td>';
									}
								if($rj->chain_length) {	
									$process_table .='<td width="150" class="nowrap"><b>Chain Length:</b>'.$rj->chain_length.' in</td>';
								}
								$process_table .= '</tr>';
								
									$process_table .= '<tr>';
								if($rj->bracelet_length) {
									$process_table .='<td width="150"><b>bracelet length : </b>'.$rj->bracelet_length.' mm</td>';
									}
								if($rj->bangle_size) {	
									$process_table .='<td width="150" class="nowrap"><b>bangle_size :</b>'.$rj->bangle_size.' mm</td>';
								}
								$process_table .= '</tr>';
								...
								$process_table .= '<tr>';
								if($rj->top_height!='') {
									$process_table .= ' <td width="125"><b>Top Height : </b>'.$rj->top_height.' mm</td>';
								}	
								if($rj->total_weight!='') {	
								...
								if($rj->metal_weight!='') {	
									$process_table .='<td width="140"><b>Metal Weight : </b>'.$rj->metal_weight.'</td>';
								}
			...
			$price_class = ($rj->total_due > 0) ? 'red' : '';
			...
			if($erp_id > 0):
			$process_table .='<input type="hidden" name="erp_id" id="erp_id" value="'.$erp_id.'" />';
			endif;
			...
			$process_table .='<input type="hidden" name="order_item_id" value="'.$order_item_id.'" />';
			$process_table .='<input type="hidden" name="order_status_id_1" value="'.$rj->order_status_id.'" />';
