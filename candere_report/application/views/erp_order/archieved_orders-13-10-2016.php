...
<ul id="buttons">
   ...
   { ?>
    ...
    { ?>
	<li class="active"><a href="<?php echo base_url('index.php/erp_order/archieved_orders')?>"><b>Shipped Orders</b></a></li>
     <?php } ?>
...
echo '<form method="POST" name="sts_form" action="'.base_url('index.php/erp_order/archieved_orders').'">';
...
<input type="hidden" name="viewtype" value="archieved">
...
<form action="<?php echo base_url('index.php/erp_order/archieved_orders'); ?>" enctype="multipart/form-data" method="post">
	<br /> 
	<input type = "submit" value = "Export Data" name="sbmt_archieved_orders" id="sbmt_to_do_list" class="styled-button-1" style="width:150px;" />
</form>
</div>
...
if($completedata)
{
	...
	foreach($completedata as $rss){
		...
		$order_item_id = $rss->order_item_id;
		...
		$complete_table .= '<tr class="'.$bgclass.' main_tr" id="'.$trCount.'"  >
			...
			<b>Order Item Id : '.$rss->order_item_id.'</b><br>
			...
			if($_allow_cancel) {			
				$complete_table .= '<form method="post" name="update_price_form" id="update_price_form_'.$trCount.'" action="'.base_url('index.php/erp_order/update_price').'" style="display:none;">
			...
			if($_allow_cancel) {	
				...
				<form method="post" name="dispatch_form" id="dispatch_form_'.$trCount.'" action="'.base_url('index.php/erp_order/update_dispatch_date').'" style="display:none;">
					...
					<input type="hidden" name="controller" value="archieved_orders">
			...
			$complete_table .='<input type="hidden" name="controller" value="archieved_orders" />';
			$complete_table .='<input type="hidden" name="order_id" value="'.$order_id.'" />';
			$complete_table .='<input type="hidden" name="order_product_id" value="'.$order_product_id.'" />';
			$complete_table .='<input type="hidden" name="order_status_id_1" value="'.$order_status_id.'" />';
			...
			$complete_table .= '<button type="button" name="btn_submit" class="styled-button-1" onclick="set_next_state('.$trCount.');">Next</button>';
