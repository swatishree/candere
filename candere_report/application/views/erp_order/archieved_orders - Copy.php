...
$(document).ready(function() {
	$("#dir_toggle").click(function(){
		...
		$.cookie("sort_dir", status_id[1], { expires: 7 });
...
<ul id="buttons">
    ...
    <li class="active"><a href="<?php echo base_url('index.php/erp_order/archieved_orders')?>"><b>Archieved Orders</b></a></li>
...
echo form_dropdown('order_dispatch_date', $options, set_value('order_dispatch_date'), 'id="order_dispatch_date" class="tb10" style="width:150px;" onchange="this.form.submit();"');
...
if($completedata)
{
	...
	foreach($completedata as $rss){
		...
		$complete_table .= '<tr class="'.$bgclass.' main_tr" id="'.$trCount.'"  >
			<td class="border left bottom" width="20" valign="top" style="padding-top:40px;"><input type="checkbox" name="chk_order"></td>	
			<td class="border bottom" width="275" style="color:#0066cc;">
			...
			if($_allow_cancel) {			
				$complete_table .= '<span style="margin-right: 20px;float: right;margin-top: -76px;"><img src="'.$edit_image.'" id="edit_price_'.$trCount.'" width="12" height="12"></span>
				<form method="post" name="update_price_form" id="update_price_form_'.$trCount.'" action="'.base_url('index.php/erp_order/update_price').'" style="display:none;">
					...
					<input type="text" name="updated_price" id="updated_price_'.$trCount.'" class="tb10" style="width:125px;">
			...
			$complete_table .= '<span>'.date("F j, Y", strtotime($rss->expected_delivery_date)).'</span>';
			...
			if($_allow_cancel) {	
				...
				<form method="post" name="dispatch_form" id="dispatch_form_'.$trCount.'" action="'.base_url('index.php/erp_order/update_dispatch_date').'" style="display:none;">
					...
					<input type="text" name="updated_dispatch_date" id="updated_dispatch_date_'.$trCount.'" class="tb10" style="width:125px;">
			...
			$complete_table .='<form method="POST" action="'.base_url('index.php/erp_order/order_save').'" name="order_update" id="order_update_'.$trCount.'" style="margin-top: 10px;">';
			...
			$complete_table .= '<textarea name="notes" class="tb10"></textarea><br>';
		...
		<table width="100%" style="border: 1px solid black;" class="diamond_detail" id="diamond_detail">
		...
		$complete_table .= '<td colspan="3" align="center" class="top_btm_padding bottom right">
			<table width="90%" style="border: 1px solid black;" name="gemstone_detail" id="gemstone_detail">
