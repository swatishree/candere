...
function save_vendor(tr_id)
{
	var form_data 	= $("#order_update_"+tr_id).serialize();
	
	$.ajax({
		type: 'POST',
		url : "<?php echo base_url()?>index.php/erp_order/save_vendor",
		data : form_data,
		success : function(result) {
			if(result=='success')
...
if($processdata)
{
	...
	foreach($processdata as $rj){
		...
		$process_table .= '<tr class="'.$bgclass.' main_tr" id="'.$trProcess.'"  >
			...
			$process_table .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" name="btn_vendor_submit" class="styled-button-1" onclick="return save_vendor('.$trProcess.');" style="width:120px;">Assign Vendor</button><br></div>';
