...
<script>
    jQuery(document).ready(function () {
        ...
        jQuery('.update').click(function () {
            ...
            price = jQuery("#bluedart_price_" + order_id + "_" + product_id).val();
            ...
            if (order_id != '' && bluedart_pin != '') {
                ...
                jQuery.ajax({
                    ...
                    data: {bluedart_pin: bluedart_pin, order_id: order_id, product_id: product_id, price: price},
                    dataType: "text",
                    cache: false,
                    success:
                            function (data) {
                ...
                jQuery.ajax({
                    ...
                    success:
                            function (data) {
                                ...
                                jQuery("#bluedart_price_" + order_id + "_" + product_id).val(json.bluedart_price);
                                jQuery("#ajax_message").html('<h2>Order Id ' + increment_id + ' Updated Successfully</h2>')
                            }
                });
...
<form name="create_batch" method="post" id="create_batch" action="<?php echo base_url(); ?>index.php/bluedartshippingprint/create_batch">
    ...
    <table width="100%" cellpadding="5" cellspacing="0">
        ...
        <tbody> 
            <?php foreach ($results as $rslt) { ?>  
                ...
                $sql = "SELECT id,awb,bluedart_pin,batch_id,product_price from bluedart_orders WHERE order_id = " . $rslt['order_id'] . " and product_id = " . $rslt['product_id'];
    ...
    <?php } else { ?>
                    ...
                    <td>
                        <input name="bluedart_pin[<?php echo $rslt['entity_id']; ?>]" id="bluedart_pin_<?php echo $rslt['order_id']; ?>_<?php echo $rslt['product_id']; ?>" value="<?php echo $awb ?>" size="10">
                    </td>  
                    <td>
                        <input name="bluedart_price[<?php echo $rslt['entity_id']; ?>]" id="bluedart_price_<?php echo $rslt['order_id']; ?>_<?php echo $rslt['product_id']; ?>" value="<?php echo $price ?>" size="10">
                    </td> 
                    <td>
                        <input type="button" name="submit" order_id="<?php echo $rslt['order_id']; ?>"  increment_id="<?php echo $rslt['order_increment_id']; ?>" product_id="<?php echo $rslt['product_id']; ?>"   class="update" value="Update Bluedart Pin and Prices"></button>
                    </td>
                    <td>
