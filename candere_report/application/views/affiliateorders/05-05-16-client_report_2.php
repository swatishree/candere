	...
	if($date_from > $date_to){
		...
		if (strpos($order_status,'canceled') !== false || strpos($order_status,'closed') !== false ) {
			...
			$sql = "SELECT sales_flat_order.increment_id AS order_id,
					 ...
					 WHERE (sales_flat_order.state IN ('$order_status') And (sales_flat_creditmemo.created_at between '$mysql_date_from' AND '$mysql_date_to')) ORDER BY sales_flat_order.entity_id DESC";
				...
				$sql = "SELECT sales_flat_order.increment_id AS order_id,
					  ...
					  FROM    (   (   sales_flat_order sales_flat_order
						   ...
						   INNER JOIN
							  sales_flat_order_payment sales_flat_order_payment
						   ON (sales_flat_order_payment.parent_id = sales_flat_order.entity_id)
					 WHERE (sales_flat_order.state IN ('$order_status') And (sales_flat_order.created_at between '$mysql_date_from' AND '$mysql_date_to')) ORDER BY sales_flat_order.entity_id DESC";
		}
