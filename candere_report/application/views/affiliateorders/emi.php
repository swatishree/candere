...
<?php
		...
		$sql = 'select distinct method from sales_flat_order_payment';
...
<h1>Select Filter for Affiliate Report</h1>
	...
	<form name="frm" method="post" id="frm" action="<?php echo $_SERVER['PHP_SELF']?>">
		<select name="method" id="method">
			...
			?>
					<option value="<?php echo $states['method'] ;?>"><?php echo Mage::getStoreConfig('payment/' .$states['method']. '/title'); ?></option>
			<?php
				}
			?>
...
<?php 
	...
	//if($_POST['method'][0]=='All'){
		//$_POST['order_status'] = array('complete','processing','new','pending_payment','closed','canceled','holded','payment_review');
		//$_POST['method'] = //array('paypal_standard','payucheckout_shared','banktransfer','cashondelivery','emi','checkmo','paytm_cc','payumoney','zaakpay','voucherpayment','cod','paywithamazon_sandbox','paywithamazon','hdfc_standard','Payzapp','Kotak','Fastbanking');
	//}
		 ...
		 $sql = "SELECT sales_flat_order.increment_id AS order_id,
          ...
          AS created_at,
			 ...
			 ON (sales_flat_order_address.parent_id = sales_flat_order.entity_id)
				   where sales_flat_order_address.address_type = 'billing' AND sales_flat_order_payment.method = '$method' And (sales_flat_order.created_at between '$mysql_date_from' AND '$mysql_date_to')
...
?>
			<tr>
				...
				<td><?php echo $rslt['customer_id'];?></td> 
				<td>
					<?php 
				...
				$readconnection = Mage::getSingleton('core/resource')->getConnection('core_read');
                 $select = $readconnection->select()->from('log_customer', array('*'))  ->where('customer_id=?',$rslt['customer_id'])   ;            
                $rowArray =$readconnection->fetchRow($select); 
				
                 

                 $select1 = $readconnection->select()->from('log_visitor', array('*'))  ->where('visitor_id=?',$rowArray['visitor_id'])   ;            
                $rowArray1 =$readconnection->fetchRow($select1); 
                   echo $rowArray1['utm_source'] ;
                  // echo 'hello' ;				   
                 ?>
