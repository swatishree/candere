...
?>  <div style="padding-bottom: 20px; float:right;">
				...
				// print_r($row);
					...
					$_productId = $_catalog->getIdBySku($img);
					 $read = Mage::getSingleton('core/resource')->getConnection('core_read');  
					  ...
					  $sql = "select DISTINCT product_id  from  pricing_table_metal_options where sku='$img'" ;
	...
	//die('okok');	
		$_Pdetails = Mage::getModel('catalog/product')->load($result);
					 
                    $sql1 = "select DISTINCT  metal_id  from  pricing_table_metal_options where sku='$img'" ;
		 $metal_id  = $read->fetchOne($sql1);
			
	 
if($metal_id == 59)
{
	$metal_image = 'white_gold_default' ;
}

if($metal_id == 60)
{
	$metal_image = 'yellow_gold_default' ;
}

if($metal_id == 61)
{
	$metal_image = 'rose_gold_default' ;
}

  $img = 	Mage::helper('catalog/image')->init($_Pdetails,$metal_image)->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize(220) ;					 
