...
$sql = $this->db->query("SELECT count(distinct(sales_flat_order.entity_id)) as count
 ...
 WHERE ( (sales_flat_order.base_total_paid > 0)
       AND  (sales_flat_order_payment.emi_amount = 0) AND (sales_flat_order.status NOT IN
            ('canceled','cancelled_as_per_customer_reques',     'cancelled_customer_not_respondin','closed','closed_refund_credited')) ) AND (Date(sales_flat_order.created_at) = '$today_curr_mnth')");
...
$sql = $this->db->query("SELECT sum(sales_flat_order.base_total_paid) as total_paid
  FROM sales_flat_order_payment sales_flat_order_payment
    INNER JOIN
        sales_flat_order sales_flat_order
    ON (sales_flat_order_payment.parent_id = sales_flat_order.entity_id)
...
$total_paid_today 	= isset($total_paid_data['total_paid']) ? $total_paid_data['total_paid'] : 0;
...
$sql = $this->db->query("SELECT count(distinct(sales_flat_order.entity_id)) as count
 ...
 WHERE ( (sales_flat_order.base_total_paid > 0)
       AND  (sales_flat_order_payment.emi_amount = 0) AND (sales_flat_order.status NOT IN
            ('canceled','cancelled_as_per_customer_reques',     'cancelled_customer_not_respondin','closed','closed_refund_credited')) ) AND (Date(sales_flat_order.created_at) BETWEEN '$first_day_current_mnth'    AND '$last_day_this_month')");
...
$total_order_value_this_month = isset($total_value_this_month['total_paid']) ? $total_value_this_month['total_paid'] : 0;
...
$sql = $this->db->query("SELECT count(distinct(sales_flat_order.entity_id)) as count
 ...
 WHERE ( (sales_flat_order.base_total_paid > 0)
       AND  (sales_flat_order_payment.emi_amount = 0) AND (sales_flat_order.status NOT IN
            ('canceled','cancelled_as_per_customer_reques',     'cancelled_customer_not_respondin','closed','closed_refund_credited')) ) AND (Date(sales_flat_order.created_at) BETWEEN '$first_day_previous_mnth'    AND '$last_day_previous_mnth')");			
...
$total_order_value_last_month = isset($total_value_last_month['total_paid']) ? $total_value_last_month['total_paid'] : 0;
...
$sql = $this->db->query("SELECT count(distinct(sales_flat_order.entity_id)) as count
 ...
 WHERE     (sales_flat_creditmemo.base_grand_total > 0)
...
$sql = $this->db->query("SELECT COUNT(DISTINCT sales_flat_order.entity_id) AS count
 ...
 WHERE (      (   sales_flat_order_payment.emi_amount > '0' )
        ...
        AND (sales_flat_order.status NOT IN
               ('canceled','cancelled_as_per_customer_reques',    'cancelled_customer_not_respondin','closed','closed_refund_credited'))  )");
...
$todays_month_emi_value 	= isset($todays_month_emi_values['total_paid']) ? $todays_month_emi_values['total_paid'] : 0;
...
$this_month_emi_value 	= isset($this_month_emi_values['total_paid']) ? $this_month_emi_values['total_paid'] : 0;
...
$last_month_emi_value 	= isset($last_month_emi_value['total_paid']) ? $last_month_emi_value['total_paid'] : 0;
