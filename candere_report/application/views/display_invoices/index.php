...
<form  name="myForm"  action="<?php echo base_url();?>index.php/display_invoices/index" onsubmit="return validateForm()" chars  method="get" accept-charset='UTF-8'>
	...
	<div style="background-position: left;    margin-top: -75px;    position: absolute;">
		...
		<input id="created_at" name="created_at" class ='input_type1' type="text" name='created_at' placeholder="Order Date" value='<?php echo $_GET['created_at']; ?>'>
...
<?php 
	...
	if($results_count == 0){ 
		...
		$csv_url = base_url('index.php/display_invoices/getCsv?todatepicker='.$_GET['todatepicker'].'&fromdatepicker='.$_GET['fromdatepicker'].'&bill_to='.$_GET['bill_to'].'&p_name='.$_GET['p_name'].'&orderid='.$_GET['orderid'].'&invoice_no='.$_GET['invoice_no'].'&created_at='.$_GET['created_at'].'  ');
