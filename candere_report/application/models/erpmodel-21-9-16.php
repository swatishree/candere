...
class ERPModel extends CI_Model 
{
 ...
 WHERE mststatus.state!='cancelled'");
...
$this->db->update($table, $data, $key);
    ...
    if($search=='')
	{
	 $search = '' ;	
		...
		$query = $this->db->query("SELECT c.id,  c.order_id,  c.sku,  c.quantity,   c.unit_price,   ed.product_image,   c.product_name,  c.quantity, ed.erp_order_id, cd.order_status_id, cd.order_item_id ,  mststatus.status_name
		FROM    (   (   (   (SELECT MAX(id) AS max_id,
                              trnorderprocessing.order_product_id
                         FROM trnorderprocessing trnorderprocessing
                       GROUP BY trnorderprocessing.order_product_id) c_max
