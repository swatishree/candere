...
class ERPModel extends CI_Model 
{
	...
	public function email_activities($search_order_id, $limit, $start)
	{
		...
		else if($param == 'sent_mail')
		{
			...
			for($j=0; $j<count($user_email); $j++)
			{
				if($content != '')
				{
					$query 		= $this->db->query("insert into email_activities (email_to, email_from, subject, content, recieved_date) values ('".$user_email[$j]."', '".$_user_id."', '".$subject."', '".$content_details."', now())");
	...
	public function tab_orders_count($status_code, $allow_status)
	{
		...
		else if($_username == 'procurement')
		{
			$updated_by = 'manufacturing';
		}
		
		if($_username == 'cad' || $_username == 'manufacturing' || $_username == 'procurement')
