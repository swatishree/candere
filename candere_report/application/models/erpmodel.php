...
class ERPModel extends CI_Model 
{
	...
	{
		$param = $_REQUEST['search_keyword'];
		$query 		= $this->db->query("select e.order_id, e.order_status, e.product_name, e.ready_image from erp_order e where e.order_id like '%".$param."%' group by order_id order by order_id desc");
		$result = $query->result();
		
		return $result;
	}
	
	public function search_value_activities()
	{
		$param = $_REQUEST['search_keyword'];
		$query 		= $this->db->query("select e.order_id, e.order_status, e.product_name, e.ready_image from erp_order e where e.order_id = '".$param."'");
		$result = $query->result();
	...
	public function email_activities($search_order_id, $limit, $start)
	{
		$whr_str			= '';
		$whr_dept			= '';
		$param 				= $_REQUEST['param'];
		$search_dept 		= $_REQUEST['search_dept'];
		$_username 			= @$this->session->userdata('_username');
		$_user_id 			= @$this->session->userdata('_user_id');		
		$email_ids_value 	= $this->input->post('email_ids_value');
		...
		if(($param == 'inbox' || $param == 'outbox') && $search_order_id != '')
		{
			$whr_str = "and (subject like '%".$search_order_id."%' || content like '%".$search_order_id."%' || recieved_date like '%".$search_order_id."%')";
		}
		
		if(($param == 'outbox') && $search_dept != '')
		{
			$whr_dept = "and email_to = '".$search_dept."'";
		}
		
		if(($param == 'inbox') && $search_dept != '')
		{
			$whr_dept = "and email_from = '".$search_dept."'";
		}
		
		$content_details = htmlspecialchars($product_details)."<br><br>".$content."<br><br>Regards<br>".ucfirst($_username)." Team";
		...
		if($param == 'inbox')
		{
			$query 		= $this->db->query("select e.*, u.username from email_activities e left join user_signup u on e.email_from=u.user_id where e.email_to='".$_user_id."' $whr_str $whr_dept order by recieved_date desc limit $start, $limit");
			$email_data = $query->result();
		}
		else if($param == 'outbox')
		{
			$query 		= $this->db->query("select e.*, u.username from email_activities e left join user_signup u on e.email_to=u.user_id where e.email_from='".$_user_id."' $whr_str $whr_dept order by recieved_date desc limit $start, $limit");
			$email_data = $query->result();
			
		}
		else if($param == 'sent_mail')
		{
			$email_data = '';
			for($j=0; $j<count($user_email); $j++)
			{
				if($content != '')
				{
					$query 		= $this->db->query("insert into email_activities (email_to, email_from, subject, content, recieved_date,priority) values ('".$user_email[$j]."', '".$_user_id."', '".$subject."', '".$content_details."', now(), '".$priority."')");
	...
	public function tab_processing_orders_count($status_code, $allow_status,$search)
	{
		$_username 	= @$this->session->userdata('_username');
		$status_code = is_array($status_code) ? $this->addQuotes(implode(",",$status_code)) : $status_code;
		...
		if($_username == 'cad' || $_username == 'manufacturing')
			$whr_dept = "and updated_by='".$_username."'";
		
	    $q = "SELECT c.order_id AS order_id,   cd.id,   cd.order_status_id,   mststatus.status_name, mststatus.sequence ,   mststatus.state  FROM    (   (   (   (SELECT MAX(id) AS max_id,  trnorderprocessing.order_status_id,  trnorderprocessing.order_product_id     FROM trnorderprocessing trnorderprocessing   GROUP BY trnorderprocessing.order_product_id) c_max   CROSS JOIN  erp_order c   ON (c_max.order_product_id = c.id))   INNER JOIN      erp_order_details b   ON (c.id = b.erp_order_id))   CROSS JOIN      trnorderprocessing cd   ON (cd.id = c_max.max_id))   INNER JOIN   mststatus mststatus   ON (cd.order_status_id = mststatus.sequence) WHERE ( mststatus.state IN ($status_code) $where $whr_dept $search)";
	...
	function delayed_orders_count($search)
	{
		$todays_date = date('Y-m-d H:i:s');
		$sql_delayed_list = "select * from erp_order WHERE dispatch_date < '".$todays_date."' and (status != '22' && status != '23' && status != '24' && status != '25') ";
		$query 		 = $this->db->query($sql_delayed_list);
		$data_count	 = $query->result();
		$this->db->last_query();
		return $data_count;		
	}
	
	function delayed_details($status_code, $allow_status, $search, $limit, $start)
	{
		if($search != '')	
		{
			$whr_str = ' and (vendor_name like "%'.$search.'%")';
		}
	...
	function vendor_details_count()
	{
		if($search != '')	
		{
			$whr_str = ' and (vendor_name like "%'.$search.'%" || vendor_email like "%'.$search.'%")';
		}
		
		$sql_vendor_list = "select * from mstvendor WHERE active=1 $whr_str";
		$query 			 = $this->db->query($sql_vendor_list);
		$vendor_data_count	 = $query->result();
		$this->db->last_query();
		return $vendor_data_count;		
	}
	
	function vendor_details($status_code, $allow_status, $search, $limit, $start)
	{
		if($search != '')	
		{
			$whr_str = ' and (vendor_name like "%'.$search.'%" || vendor_email like "%'.$search.'%")';
		...
		$whr_lmt = ' limit '.$start.', '.$limit.'';
		
		$sql_vendor_list = "select * from mstvendor WHERE active=1 $whr_str $whr_lmt";
		$query 			 = $this->db->query($sql_vendor_list);
		$vendor_data	 = $query->result();		
		
		$this->db->last_query();
		
		if($_REQUEST['sbmt_vendor_list'] == 'Export Data')
		{		
			$this->load->dbutil();
			...
			$delimiter = ",";
			$newline = "\r\n";
			$filename = "vendor_lists.csv";
			
			$vendor_export_list = "select v.vendor_name, v.vendor_email, o.order_id, o.vendor_assigned_date, o.vendor_recieved_date, c.name, c.sku from mstvendor v left join trnorderprocessing o on v.vendor_id=o.vendor_id left join catalog_product_flat_1 c on o.order_product_id=c.entity_id  WHERE v.active=1 and (o.order_status_id='9') $whr_str order by id";
			
			$query_export_list  = $this->db->query($vendor_export_list);
			
			$data = $this->dbutil->csv_from_result($query_export_list, $delimiter, $newline);
	...
	function cad_order_details($status_code, $allow_status, $search, $sort, $dir, $limit, $start)
	{   
		$_username 	= @$this->session->userdata('_username');
		if($status_code!=''){
			$status_code = is_array($status_code) ? $this->addQuotes(implode(",",$status_code)) : $status_code;
 ...
 //WHERE 1
		...
		if($_REQUEST['sbmt_cancelled_order'] == 'Export Data' || $_REQUEST['sbmt_completed_order'] == 'Export Data' || $_REQUEST['sbmt_to_do_list'] == 'Export Data' || $_REQUEST['sbmt_processing_orders'] == 'Export Data' || $_REQUEST['sbmt_archieved_orders'] == 'Export Data' )
		{
			
			$export_query = "SELECT c.order_id AS order_id, c.mktplace_name , c.order_item_id, c.agent_name, c.customer_name, c.customer_lastname, c.customer_email, c.shipping_country, c.sku, c.sku_custom , c.product_name, c.quantity, c.unit_price, c.updated_price, c.shipping_amount, c.total_due, c.expected_delivery_date, c.dispatch_date, c.discount_amount, cd.vendor_id, cd.notes, cd.order_product_id, cd.greeting_card_id, cd.personal_message, cd.website_id, cd.updated_date, cd.updated_by, b.product_type, b.engrave_message, b.engrave_font,b.description, b.expected_date_append , b.metal, b.purity, b.height, b.width, b.design_identifier, b.top_thickness, b.top_height, b.bottom_thickness, b.metal_weight, b.total_weight, b.no_of_stones, b.product_image, b.product_url, b.chain_thickness, b.chain_length, b.bracelet_length, b.bangle_size, b.kada_size, b.ring_size, b.diamond_1_status, b.diamond_2_status, b.diamond_3_status, b.diamond_4_status, b.diamond_5_status, b.diamond_6_status, b.diamond_7_status, b.diamond_1_stones, b.diamond_2_stones, b.diamond_3_stones, b.diamond_4_stones, b.diamond_5_stones, b.diamond_6_stones, b.diamond_7_stones, b.diamond_1_weight, b.diamond_2_weight, b.diamond_3_weight, b.diamond_4_weight, b.diamond_5_weight, b.diamond_6_weight, b.diamond_7_weight, b.diamond_1_clarity, b.diamond_2_clarity, b.diamond_3_clarity, b.diamond_4_clarity, b.diamond_5_clarity, b.diamond_6_clarity, b.diamond_7_clarity, b.diamond_1_shape, b.diamond_2_shape, b.diamond_3_shape, b.diamond_4_shape, b.diamond_5_shape, b.diamond_6_shape, b.diamond_7_shape, b.diamond_1_color, b.diamond_2_color, b.diamond_3_color, b.diamond_4_color, b.diamond_5_color, b.diamond_6_color, b.diamond_7_color, b.diamond_1_setting_type, b.diamond_2_setting_type, b.diamond_3_setting_type, b.diamond_4_setting_type, b.diamond_5_setting_type, b.diamond_6_setting_type, b.diamond_7_setting_type, b.gem_1_status, b.gem_2_status, b.gem_3_status, b.gem_4_status, b.gem_5_status, b.gemstone_1_stone, b.gemstone_2_stone, b.gemstone_3_stone, b.gemstone_4_stone, b.gemstone_5_stone, b.gemstone_1_type, b.gemstone_2_type, b.gemstone_3_type, b.gemstone_4_type, b.gemstone_5_type, b.gemstone_1_color, b.gemstone_2_color, b.gemstone_3_color, b.gemstone_4_color, b.gemstone_5_color, b.gemstone_1_shape, b.gemstone_2_shape, b.gemstone_3_shape, b.gemstone_4_shape, b.gemstone_5_shape, b.gemstone_1_setting_type, b.gemstone_2_setting_type, b.gemstone_3_setting_type, b.gemstone_4_setting_type, b.gemstone_5_setting_type, b.gemstone_1_weight, b.gemstone_2_weight, b.gemstone_3_weight, b.gemstone_4_weight, b.gemstone_5_weight, c.buyer_address, c.order_placed_date, b.erp_order_id, c.customer_id,mststatus.sequence, mststatus.status_name, mststatus.state
		FROM ((((SELECT MAX(id) AS max_id, trnorderprocessing.order_status_id,trnorderprocessing.order_product_id FROM trnorderprocessing trnorderprocessing
                       GROUP BY trnorderprocessing.order_product_id) c_max CROSS JOIN erp_order c ON (c_max.order_product_id = c.id)) INNER JOIN erp_order_details b ON (c.id = b.erp_order_id)) CROSS JOIN trnorderprocessing cd ON (cd.id = c_max.max_id)) INNER JOIN mststatus mststatus ON (cd.order_status_id = mststatus.sequence) WHERE $status_in $search $where $whr_dept order by $sort $dir";
			$query 		= $this->db->query($export_query);
			...
			if($_REQUEST['sbmt_cancelled_order'] == 'Export Data')
				$filename = "cancelled_products.csv";
			else if($_REQUEST['sbmt_completed_order'] == 'Export Data')
				$filename = "completed_products.csv";
			else if($_REQUEST['sbmt_to_do_list'] == 'Export Data')
				$filename = "to_do_lists.csv";
			else if($_REQUEST['sbmt_processing_orders'] == 'Export Data')
				$filename = "processing_orders.csv";
			else if($_REQUEST['sbmt_archieved_orders'] == 'Export Data')
				$filename = "shipped_orders.csv";
	...
	function order_details($status_code, $allow_status, $search, $sort, $dir, $limit, $start)
	{
		...
		if($_username == 'cad' || $_username == 'manufacturing')
		 ...
		 $q = "SELECT c.order_id AS order_id, c.mktplace_name , c.order_item_id , c.mktplace_order_id , c.mktplace_sub_order_id , cd.id, cd.order_status_id, c.product_id, c.agent_name, c.customer_name, c.customer_lastname, c.customer_email, c.shipping_country, c.sku, c.sku_custom , c.product_name, c.quantity, c.unit_price, c.updated_price, c.order_currency_code , c.shipping_amount, c.total_due, c.expected_delivery_date, c.dispatch_date, c.discount_amount, cd.vendor_id, cd.notes, cd.order_product_id, cd.greeting_card_id, cd.personal_message, cd.website_id, cd.updated_date, cd.updated_by, b.product_type, b.engrave_message, b.engrave_font,b.description, b.expected_date_append , b.metal, b.purity, b.height, b.width, b.design_identifier, b.top_thickness, b.top_height, b.bottom_thickness, b.metal_weight, b.total_weight, b.no_of_stones, b.product_image, b.product_url, b.chain_thickness, b.chain_length, b.bracelet_length, b.bangle_size, b.kada_size, b.ring_size, b.diamond_1_status, b.diamond_2_status, b.diamond_3_status, b.diamond_4_status, b.diamond_5_status, b.diamond_6_status, b.diamond_7_status, b.diamond_1_stones, b.diamond_2_stones, b.diamond_3_stones, b.diamond_4_stones, b.diamond_5_stones, b.diamond_6_stones, b.diamond_7_stones, b.diamond_1_weight, b.diamond_2_weight, b.diamond_3_weight, b.diamond_4_weight, b.diamond_5_weight, b.diamond_6_weight, b.diamond_7_weight, b.diamond_1_clarity, b.diamond_2_clarity, b.diamond_3_clarity, b.diamond_4_clarity, b.diamond_5_clarity, b.diamond_6_clarity, b.diamond_7_clarity, b.diamond_1_shape, b.diamond_2_shape, b.diamond_3_shape, b.diamond_4_shape, b.diamond_5_shape, b.diamond_6_shape, b.diamond_7_shape, b.diamond_1_color, b.diamond_2_color, b.diamond_3_color, b.diamond_4_color, b.diamond_5_color, b.diamond_6_color, b.diamond_7_color, b.diamond_1_setting_type, b.diamond_2_setting_type, b.diamond_3_setting_type, b.diamond_4_setting_type, b.diamond_5_setting_type, b.diamond_6_setting_type, b.diamond_7_setting_type, b.gem_1_status, b.gem_2_status, b.gem_3_status, b.gem_4_status, b.gem_5_status, b.gemstone_1_stone, b.gemstone_2_stone, b.gemstone_3_stone, b.gemstone_4_stone, b.gemstone_5_stone, b.gemstone_1_type, b.gemstone_2_type, b.gemstone_3_type, b.gemstone_4_type, b.gemstone_5_type, b.gemstone_1_color, b.gemstone_2_color, b.gemstone_3_color, b.gemstone_4_color, b.gemstone_5_color, b.gemstone_1_shape, b.gemstone_2_shape, b.gemstone_3_shape, b.gemstone_4_shape, b.gemstone_5_shape, b.gemstone_1_setting_type, b.gemstone_2_setting_type, b.gemstone_3_setting_type, b.gemstone_4_setting_type, b.gemstone_5_setting_type, b.gemstone_1_weight, b.gemstone_2_weight, b.gemstone_3_weight, b.gemstone_4_weight, b.gemstone_5_weight, c.buyer_address, c.order_placed_date, b.erp_order_id, c.customer_id,mststatus.sequence, mststatus.status_name, mststatus.state
		FROM ((((SELECT MAX(id) AS max_id, trnorderprocessing.order_status_id,trnorderprocessing.order_product_id FROM trnorderprocessing trnorderprocessing
                       GROUP BY trnorderprocessing.order_product_id) c_max CROSS JOIN erp_order c ON (c_max.order_product_id = c.id)) INNER JOIN erp_order_details b ON (c.id = b.erp_order_id)) CROSS JOIN trnorderprocessing cd ON (cd.id = c_max.max_id)) INNER JOIN mststatus mststatus ON (cd.order_status_id = mststatus.sequence) WHERE $status_in $search $where $whr_dept order by $sort $dir limit $start, $limit";
 ...
 //WHERE 1
	...
	function getStatusName($sequence)
	{
		$query = $this->db->query('SELECT status_name , sequence FROM mststatus where sequence='.$sequence.'');
	...
	function getStatus()
	{
		$query = $this->db->query('SELECT status_name, sequence FROM mststatus');
	...
	function getAllStatus()
    {
		...
		$query = $this->db->query("SELECT status_id, sequence, status_name FROM mststatus where sequence IN $sequence order by sequence asc");
    ...
    {
        $query = $this->db->query('SELECT status_id, sequence, status_name FROM mststatus where sequence IN ('.$allow_status.') and cancelled!=1 order by sequence ASC');
        return $query->result();
    }
	
	function status_per_user($allow_status)
    {
        $query = $this->db->query('SELECT status_id, sequence , status_name FROM mststatus where sequence IN ('.$allow_status.')  order by sequence ASC');
        return $query->result();
    }
	...
	function getOrderData($order_id,$order_product_id)
    {
        $query = $this->db->query("SELECT id, order_id, order_product_id , order_status_id, vendor_id, notes, greeting_card_id, personal_message, website_id FROM trnorderprocessing where order_id='$order_id' and order_product_id = '$order_product_id' order by id DESC");
        return $query->result();
    }
    ...
    { 
		...
		public function non_finished_count($status_code,$search)
	{
		
			if($status_code!=''){
			$status_code = is_array($status_code) ? $this->addQuotes(implode(",",$status_code)) : $status_code;
			$status_in = "mststatus.state NOT IN ($status_code)";
		} else {
			$status_in = "1";
		}
    ...
    if($search=='')
	{
	 $search = '' ;	
       ...
       INNER JOIN
          mststatus mststatus
       ON (cd.order_status_id = mststatus.sequence)
 WHERE $status_in $search");
		...
		return count($unfinished_data);
			...
			public function tab_orders_count1($status_code,$search)
	{
 ...
 WHERE $status_in $search");
	...
	public function getNonFinishedProducts($status_code,$search,$limit, $start, $sort ,$dir)
	{
		if($status_code!='')
		{
			$status_code = is_array($status_code) ? $this->addQuotes(implode(",",$status_code)) : $status_code;
		...
		if($search=='')
		{
		 $search = '' ;	
			...
			$query = $this->db->query("SELECT c.id,  c.order_id, c.mktplace_name ,  c.sku,  c.quantity,   c.unit_price,   ed.product_image,   c.product_name,  c.quantity, ed.erp_order_id, cd.order_status_id, cd.order_item_id ,  mststatus.status_name
			FROM    (   (   (   (SELECT MAX(id) AS max_id,
								  trnorderprocessing.order_product_id
							 FROM trnorderprocessing trnorderprocessing
						   GROUP BY trnorderprocessing.order_product_id) c_max
		   ...
		   INNER JOIN
			  mststatus mststatus
		   ON (cd.order_status_id = mststatus.sequence)
			WHERE $status_in $search  order by $sort $dir  limit $start, $limit");
			if($_REQUEST['sbmt_order'] == 'Export Data')
			{
				$this->load->dbutil();
				$this->load->helper('file');
				$this->load->helper('download');
				...
				$result = $this->db->query("SELECT c.id, c.order_id, c.mktplace_name, c.sku, c.unit_price,c.product_name,c.quantity, cd.order_item_id ,  mststatus.status_name
				FROM    (   (   (   (SELECT MAX(id) AS max_id,
									  trnorderprocessing.order_product_id
								 FROM trnorderprocessing trnorderprocessing
							   GROUP BY trnorderprocessing.order_product_id) c_max
			   ...
			   INNER JOIN
				  mststatus mststatus
			   ON (cd.order_status_id = mststatus.sequence)
				WHERE $status_in $search  order by $sort $dir");
