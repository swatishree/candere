...
ORDER BY sales_flat_invoice_grid.order_id DESC"; 
	...
	{ 
		...
		$this->zend->load('Zend/Barcode');
		 ...
		 $barcodeOptions = array('text' => $code, 'width'=> $width);  //echo "<br>";
		
		//print_r($barcodeOptions);
		$rendererOptions = array('width'=> $width); 


		$img = Zend_Barcode::draw('code39', 'image', array('text' => $code),$barcodeOptions, $rendererOptions); //die('yesttt');
	...
	{ 
		...
		if($query->num_rows() == 0){ 
			...
			if($_order->getPayment()->getMethodInstance()->getCode() == 'cod'){
				$sql =	"SELECT id,awb_no FROM bluedart_cod_awb where is_used = '0' LIMIT 1"; 
			}else {
			...
			if($_order->getPayment()->getMethodInstance()->getCode() == 'cod'){
				$sql =	"update bluedart_cod_awb set is_used = '1' where id = $bluedart_pin_id LIMIT 1"; 
			}else {
        ...
        $data_return['bluedart_price'] 	= $product_price ;
	...
	{ 
		...
		foreach($results as $key=>$rslt){
			$_order = Mage::getModel('sales/order')->load($rslt['order_id']);
                                                ...
                                                if($_order->getPayment()->getMethodInstance()->getCode() == 'cod'){
                                                    $customer_code = '367301';
                                                     $cod = 'COD';
                                                }else{
                                                    $cod = 'NONCOD';
                                                    $customer_code = '347314';
                                                }
			...
			$data[($key + 1)][14] = '0';
			$data[($key + 1)][15] = '0';
			$data[($key + 1)][16] = $customer_code;
			$data[($key + 1)][18] = 'Enovate Lifestyles Private Limited';
			$data[($key + 1)][19] = '601-602, Om Shakti Samrat CHS Ltd.';
			$data[($key + 1)][20] = 'Plot No. 21,Shakti Niwas,Ramchandra Lane extn,Near Greater Bombay Co-op Bank,Kanchpada';
	...
	//$this->email->to('shashank.sharma@candere.com', 'Shashank Sharma'); 
		...
		$this->email->to('rupesh.jain@candere.com', 'Rupesh Jain'); 
	...
	{ 
		...
		foreach($results as $key=>$rslt){
			...
			if($_order->getPayment()->getMethodInstance()->getCode() == 'cod'){ 
				...
				$collectable_value =(int)$_order->getTotalDue();
				$price = 0;
                    
            }else{
                $collectable_value = 0;
				$price = $rslt['product_price'];
			...
			$data[($key + 1)][13] = '0.5';
			$data[($key + 1)][14] = $price;
			$data[($key + 1)][15] = $collectable_value;
			$data[($key + 1)][16] = $customer_code;
			$data[($key + 1)][18] = 'Enovate Lifestyles Private Limited';
			$data[($key + 1)][19] = '601-602, Om Shakti Samrat CHS Ltd.';
			$data[($key + 1)][20] = 'Plot No. 21,Shakti Niwas,Ramchandra Lane extn,Near Greater Bombay Co-op Bank,Kanchpada';
			...
			$data[($key + 1)][25] = '5';
			$data[($key + 1)][26] = 1;
			$data[($key + 1)][27] = 'BOM'.$customer_code;
		}
