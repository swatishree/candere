...
class bluedartshippingprint extends CI_Controller { 
  ...
  FROM    (   sales_flat_invoice_grid sales_flat_invoice_grid
           INNER JOIN
       ...
       ON (sales_flat_order_grid.entity_id = sales_flat_invoice_grid.order_id)
 WHERE sales_flat_order_grid.status NOT IN ('closed','complete','pending_payment','pending')
 ...
 WHERE     (sales_flat_order_grid.entity_id = $order_id)
	...
	{ 
		...
		foreach($results as $key=>$rslt){
			...
			$data[($key + 1)][19] = 'E-3, Nutan Ayojan Nagar, X Road No. 4';
			$data[($key + 1)][20] = 'Liberty Garden, Malad West';
	...
	{ 
		...
		foreach($results as $key=>$rslt){
			...
			$data[($key + 1)][10] = $telephone;
			$data[($key + 1)][11] = 'NA';
			$data[($key + 1)][12] = 'Imitation Jewellery';
			$data[($key + 1)][13] = '0.5';
			$data[($key + 1)][14] = '0';
			$data[($key + 1)][15] = '0';
			$data[($key + 1)][16] = '347314';
			$data[($key + 1)][18] = 'Enovate Lifestyles Private Limited';
			$data[($key + 1)][19] = '602, 6th Floor, Agarwal B2B';
			$data[($key + 1)][20] = 'Above Silver Oaks Restaurant,Kanchpada, Malad West';
			$data[($key + 1)][21] = 'Mumbai, Maharashtra';
			$data[($key + 1)][22] = '400064';
			$data[($key + 1)][23] = '18';
			$data[($key + 1)][24] = '15';
