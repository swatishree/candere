...
class Affiliateorders extends CI_Controller  {
	...
	public function emi1()
	{ 
		$this->db->cache_delete_all();
		$this->load->helper('csv');
		$message_arr = array('');
		$this->load->view('templates/header'); 
        $this->load->view("affiliateorders/emi_reminder",$message_arr);
		$this->load->view('templates/footer');
	}
	
	public function emi_download()
	{
		$this->load->dbutil();
        $this->load->helper('file');
        $this->load->helper('download');
        ...
        $newline = "\r\n";
 		   ...
 		   $query  = "select  a.increment_id as orderid , a.status , a.grand_total , a.total_paid ,  a.created_at , b.method from sales_flat_order a left join sales_flat_order_payment b 
		   on
           a.entity_id = b.parent_id 
		   where a.status NOT IN ('canceled','pending_payu_payment') and  b.method = 'emi' and  a.created_at between '$mysql_date_from' AND '$mysql_date_to' order by a.entity_id ASC ";
		   ...
		   $resultarray 	= $result->result_array();
			...
			$this->load->helper('csv'); 
			$this->load->dbutil(); 
			array_to_csv($resultarray,'emi_'.date('d-M-Y').'.csv');
