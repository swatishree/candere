...
class Export_diamond_size_wise extends CI_Controller  {
	...
	{ 
        ...
        $this->load->view('export_diamond_size_wise/index',$message_arr);
	...
	public function export_chains_bracelets(){
		...
		if($sql){
					...
					foreach($attribute_collection->getSource()->getAllOptions(true, true) as $attribute_collect) { 
						if($attribute_collect['label'] !=''){
							...
							if( ($sigh_price == 0) && ($siij_price == 0) && ($vsgh_price == 0) && ($vvsef_price == 0) ) {
								$customized_design_pricing_sigh = $this->getCustomizedDesignPricing($product_id,$metal_id,$purity_id,$selected_size,$chain_thickness=0,$diamond_selection=0,$offer_percent,$base_purity_id);
							...
							unset($metal_weight);
							unset($total_weight);
							unset($siij_old_price,$siij_new_price);
							unset($sigh_old_price,$sigh_new_price);
							unset($vsgh_old_price,$vsgh_new_price);
							unset($vvsef_old_price,$vvsef_new_price);
						}						
					}
			...
			redirect('export_diamond_size_wise/index', 'refresh');
		...
		if($sql){
			...
			foreach($sql as $value){
				...
				if($gold_product == 0) {
					...
					if( ($sigh_price == 0) && ($siij_price == 0) && ($vsgh_price == 0) && ($vvsef_price == 0) ) {
						$customized_design_pricing_sigh = $this->getCustomizedDesignPricing($product_id,$metal_id,$purity_id,$selected_size=0,$chain_thickness=0,$diamond_selection=0,$offer_percent,$base_purity_id);
