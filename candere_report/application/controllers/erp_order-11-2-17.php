...
class Erp_Order extends Auth_Controller  {
	...
	{		
		$data['selectdata'] 	= $this->erpmodel->search_activities();		
		$result = '';
		if(count($data['selectdata']) > 0)
		{
			foreach($data['selectdata'] as $row)
			{
				$search_values = $row->order_status."***".$row->product_name."***".$row->ready_image;
				$result .= '<div class="show" align="left"><span class="country_name">'.$row->order_id.'</span></div>';
			}
		}
		else
		{
			$result = '<div class="show" align="left">No matching records.</div>';
		}
	...
	{		
		$data['selectdata'] 	= $this->erpmodel->search_value_activities();		
		$result = '';
		$j = 1;
		if(count($data['selectdata']) > 0)
		{
			$search_values = '<table width="80%" cellspacing="0" cellpadding="0" border="0"><tr><td colspan="4" border="0"><b>Hello Team,</b></td></tr><tr><td colspan="4">&nbsp;</td></tr><tr><td colspan="4">Order No - '.$data['selectdata'][0]->order_id.'</td></tr><tr><td colspan="4">&nbsp;</td></tr>
			<tr><td style="width:40px;"><b>No.</b></td><td style="width:100px;"><b>Status</b></td><td style="width:100px;"><b>Product Name</b></td><td style="width:100px;"><b>Ready Image</b></td></tr><tr><td colspan="4">&nbsp;</td></tr>';
			foreach($data['selectdata'] as $row)
			{
				$search_values .= '<tr><td style="width:40px;">'.$j.'</td><td style="width:100px;">'.$row->order_status.'</td><td style="width:100px;">'.$row->product_name.'</td><td style="width:200px;">';
				if($row->ready_image != '') {
				$search_values .= '<img src="'.$row->ready_image.'" width="50px;" height="50px;">';
				}
				$search_values .= '</td></tr><tr><td colspan="4">&nbsp;</td></tr>';
				$j++;
			}
			$search_values .= '</table>';
		}
	...
	public function saveemaildetails()
	{
		...
		$row_data['priority'] 		= $this->input->post('priority');
		$row_data['content'] 		= "Priority : ".$row_data['priority']."<br>Order No. : ".$order_id."<br> Product Name : ".$product_name."<br> ".$this->input->post('content');
		$row_data['subject'] 		= $this->input->post('subject');
		$row_data['email_from']		= @$this->session->userdata('_user_id');
		$row_data['recieved_date']	= date('Y-m-d');
		
		for($j=0; $j<count($user_email); $j++)
		{
			if($row_data['content'] != '')
			{
				$row_data['email_to'] 		= $user_email[$j];
				//$query 		= $this->db->query("insert into email_activities (email_to, email_from, subject, content, recieved_date) values ('".$user_email[$j]."', '".$_user_id."', '".$subject."', '".$row_data['content']."', now())");
				$this->erpmodel->AddNewRow('email_activities', $row_data);
			}
		}
	...
	public function send_email()
	{
		...
		$recordsCount 	= $this->erpmodel->vendor_details_count();
		
		$config['base_url'] 	= base_url()."index.php/erp_order/send_email?param=".$_REQUEST['param'];
		$config['total_rows'] 	= count($recordsCount);
		$config['per_page'] 	= 20; 
		...
		/*echo "--ss--".$start = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		
		$start = ($start !== 0) ? (int)$start : 1;
		$start = (($start - 1) * $limit);*/
		
		$start=($_REQUEST['per_page']) ? $_REQUEST['per_page'] : 0;
		...
		if($search_by!='')
		{
			...
			$search_by = implode(',',$array);
			$search .= " and c.mktplace_name IN($search_by)";
			$this->session->set_userdata('search_by', $search_by);
		}
		elseif($this->session->userdata('search_by'))
		{
		...
		$data['selectdata'] 	= $this->erpmodel->email_activities($search_order_id, $limit, $start);
		//$data = array();
		$this->load->view('templates/header');
		$this->load->view('erp_order/send_email',$data);	
		$this->load->view('templates/footer');
	}
	
	public function ready_image()
	{        
		$erp_order_id = $this->input->post("erp_order_id");
		$order_id     = $this->input->post("order_id");
		
        if (!$this->upload->do_upload('ready_image'))
		{
			$error = array('error' => $this->upload->display_errors()); 
			
			/*$this->load->view('templates/header');
			$this->load->view('erp_order/product_updates',$error);	
			$this->load->view('templates/footer');*/
			redirect(base_url('index.php/erp_order/product_updates?param=error'));
		}			
        else
		{			 
			$imagedata = array('upload_data' => $this->upload->data()); 
			$imagepath = array('ready_image' => base_url() .'uploads/'. $imagedata['upload_data']['file_name']);
			
			$this->db->where(array('order_id'=>$order_id, 'id'=>$erp_order_id ));
			$this->db->update('erp_order',$imagepath);
			
			redirect(base_url('index.php/erp_order/product_updates?param=success'));
		}
	...
	public function delayed()
	{
		...
		$search_by_status 		= implode(',',$this->input->post('search_by_status'));
		if($search_by_status)
		{
			$search .= " and c.status IN($search_by_status)";
			$this->session->set_userdata('search_by_status', $search_by_status);		
		}
		elseif($this->session->userdata('search_by_status'))
		{
			$search_by_status = $this->session->userdata('search_by_status');
			
		}
		else
		{
			$search_by_status ="";
		 $this->session->unset_userdata('search_by_status');   
		}
	
		if($search_order_id != '')
		{
			$search .= " and order_id like '$search_order_id' ";
		}
		...
		//$recordsCount		= $this->erpmodel->delayed_orders_count($search);
		$recordsCount = 120;
		$config['base_url'] 	= base_url()."index.php/erp_order/delayed";
		...
		$data['processdata'] 	= $this->erpmodel->delayed_details($array_processing, $allow_status, $search, $sort, $dir, $limit, $start);
		$this->load->view('templates/header');
		$this->load->view('erp_order/delayed_delivery',$data);	
		$this->load->view('templates/footer');
	}
	...
	public function vendor_list()
	{
		...
		$config['base_url'] 	= base_url()."index.php/erp_order/vendor_list";
		$config['total_rows'] 	= count($recordsCount);
		$config['per_page'] 	= 20; 
		$config['uri_segment'] 	= 3;
		$config['full_tag_open'] = "<div class=\"pagination\">\n<ul>\n";
		...
		$data['selectdata'] 	= $this->erpmodel->vendor_details($array_pending='', $allow_status, $search_order_id, $limit, $start);
		$this->load->view('templates/header');
		$this->load->view('erp_order/vendor_list',$data);	
		$this->load->view('templates/footer');
	}
	...
	public function index()
	{
	 ...
	 public function imageupload() { 
	  ...
	  public function exportdata()
	  {
		...
		erp_order_details.engrave_font,
	...
	{
		$array_pending 		= array('processing');
		$allow_status 		= $this->session->userdata('order_status');
		//print_r($allow_status);
		$recordsCount		= $this->erpmodel->tab_orders_count($array_pending,$allow_status);
		
		$config['base_url'] 	= base_url()."index.php/erp_order/to_do";
		$config['total_rows'] 	= $recordsCount;
		$config['per_page'] 	= 10; 
		$config['uri_segment'] 	= 3;
		$config['full_tag_open'] = "<div class=\"pagination\">\n<ul>\n";
		...
		$data['selectdata'] 	= $this->erpmodel->cad_order_details($array_pending, $allow_status, $search, $sort ,$dir , $limit, $start);
		
		$this->load->view('templates/header');
		$this->load->view('erp_order/to_do',$data);	
		$this->load->view('templates/footer');
	} 
	...
	public function to_do_list()
	{
		...
		public function reset_search()
		{
				...
				if($viewtype=='complete')
			{
				// echo 'hello complete' ;
				// exit ;
				redirect(base_url('index.php/erp_order/completed_orders'));
			}
			if($viewtype=='cancelled')
			{
			   redirect(base_url('index.php/erp_order/cancelled_orders'));
			...
			if($viewtype=='updates')
			{
				redirect(base_url('index.php/erp_order/product_updates'));
			}
			
			if($viewtype=='processing')
			{
				 
				redirect(base_url('index.php/erp_order/processing_orders'));
			}
			if($viewtype=='archieved')
			{
				 
				redirect(base_url('index.php/erp_order/archieved_orders'));
			}
		}
		...
		{		 
			$array_cancelled 	= array('cancelled','complete');		
			$search_order_id 	= $this->input->post('search_order_id');
			$search_by 			= $this->input->post('search_by');
			if($search_by!='')
			{
			...
			$recordsCount = $this->erpmodel->non_finished_count($array_cancelled,$search);
			...
			$data['prod_updates'] 	= $this->erpmodel->getNonFinishedProducts($array_cancelled,$search, $limit, $start, $sort ,$dir);
	...
	public function cancelled_orders()
	{
		...
		$allow_status 		= '';		
		$recordsCount		= $this->erpmodel->tab_orders_count1($array_cancelled,$search);
	...
	public function completed_orders()
	{
		...
		$recordsCount		= $this->erpmodel->tab_orders_count1($array_complete,$search);
	...
	public function processing_orders()
	{
		...
		$recordsCount		= $this->erpmodel->tab_processing_orders_count($array_processing,$allow_status,$search);
	...
	public function order_save()
	{
		...
		if($lastinsertedid) {
			if($this->input->post('order_product_id')) {
				...
				$data['erp_id'] 			= $this->input->post('erp_id');
				if($row_data['vendor_id'] > 0 && $rj['status'] == 9)
				{
					$data['vendor_assigned_date'] 	= date('Y-m-d');
					$sql_upd_sent = "update trnorderprocessing set vendor_id='".$row_data['vendor_id']."', vendor_assigned_date='".$data['vendor_assigned_date']."', updated_by='".$row_data['updated_by']."' where id='".$lastinsertedid."' and order_status_id = 9 and order_id='".$row_data['order_id']."' and order_item_id='".$row_data['order_item_id']."' and order_product_id='".$row_data['order_product_id']."'";
					mysql_query($sql_upd_sent);
				}
				
				if($row_data['vendor_id'] > 0 && $rj['status'] == 18)
				{
					$data['vendor_recieved_date'] 	= date('Y-m-d');
					$sql_upd_received = "update trnorderprocessing set vendor_id='".$row_data['vendor_id']."', vendor_recieved_date='".$data['vendor_recieved_date']."', updated_by='".$row_data['updated_by']."' where order_status_id = 9 and order_id='".$row_data['order_id']."' and order_item_id='".$row_data['order_item_id']."' and order_product_id='".$row_data['order_product_id']."'";
					mysql_query($sql_upd_received);
				}
	...
	public function saveproductweightdetails()
	{
		$row_data['gold_weight'] 		= $this->input->post('gold_weight');
		$row_data['diamond_weight'] 	= $this->input->post('diamond_weight');
		$row_data['gemstone_weight'] 	= $this->input->post('gemstone_weight');
		$row_data['id'] 				= $this->input->post('id');
	...
	public function send_mail()
	{
		...
		$data['selectdata'] = $this->erpmodel->get_order_details($erp_order_id,$erp_product_id);
		$this->load->view('templates/header');
		$this->load->view('erp_order/send_mail',$data);
		$this->load->view('templates/footer');
	}
	...
	public function weight_update()
	{
		...
		$data['selectdata'] = $this->erpmodel->get_order_details($erp_order_id,$erp_product_id);
		$this->load->view('templates/header');
		$this->load->view('erp_order/weight_update',$data);
		$this->load->view('templates/footer');
	}
	...
	public function export_product_updates()
	{$erp_order_id = $this->input->post("erp_order_id");
       $order_id = $this->input->post("order_id");
		$this->load->dbutil();
