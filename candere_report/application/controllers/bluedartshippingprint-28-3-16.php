	...
	{ 
		...
		foreach($results as $key=>$rslt){
			...
			if($_order->getPayment()->getMethodInstance()->getCode() == 'cod'){ 
				...
				$collectable_value = Mage::app()->getLocale()->currency($currentCurrencyCode)->toCurrency((int)$_order->getTotalDue());
				$price = 0;
                    
            }else{
                $collectable_value = 0;
				$price = Mage::app()->getLocale()->currency($currentCurrencyCode)->toCurrency($rslt['product_price']);
