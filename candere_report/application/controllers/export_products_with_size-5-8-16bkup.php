	...
	public function export_products_without_dropdown(){
		...
		if($sql){
			...
			foreach($sql as $value){
				...
				$array[] = array('sku'=>$sku, 'metal'=>$metal, 'purity'=>$purity, 'name'=>$name, 'metal_weight'=>$metal_weight, 'total_weight'=>$total_weight, 'product_type'=>$product_type, 'old_price'=>$old_price, 'price_after_discount'=>$new_price); 
	...
	public function export()
	{
		...
		if($sql){
			...
			foreach($sql as $key=>$value){
				...
				$base_new_price    = $product_price_details['old_price'];
				...
				foreach($attribute_collection->getSource()->getAllOptions(true, true) as $attribute_collect) { 
					if($attribute_collect['label'] !=''){  
						echo "lab--".$label 					= $attribute_collect['label'] ;
						$explode_label 			= explode("&nbsp;", $label);
						$selected_size_label 	= trim(end($explode_label)) ; 
						$selected_size 			= trim(str_replace('ANNA','',str_replace('(in)','',str_replace('mm','',end($explode_label)))));
						...
						//$customized_design_pricing = $this->getCustomizedDesignPricing($product_id,$metal_id,$purity_id,$selected_size,$chain_thickness=0,$diamond_selection=0,$offer_percent,$base_purity_id);
						
						//$customized_design_pricing  = Mage::helper('function')->getCustomizedDesignPricing($product_id,$metal_id,$purity_id,$selected_size,$chain_thickness=0,$diamond_selection,$purity_id,$clarity,$color);
						...
						$total_weight = number_format(((($base_weight / $multiply_factor) * $selected_size) - $base_weight) + $base_weight,2);
						...
						if(trim($value['purity']) == '14K')
							...
							$purity_id = 538;
						$customized_design_pricing  = Mage::helper('function')->getCustomizedDesignPricing($product_id,$value['metal_id'],$purity_id,$label,$thickness,$diamond_selection,$purity_id,$clarity,$color);
						
						echo "<pre>";
						print_r($customized_design_pricing);
						echo "</pre>";
						...
						/*$array[] = array('sku'=>$sku, 'metal'=>$metal, 'purity'=>$purity, 'name'=>$name,'selected_length'=>$selected_length, 'selected_size_label'=>$selected_size_label, 'size_in_inches'=>$size_in_inches, 'metal_weight'=>$metal_weight, 'total_weight'=>$total_weight, 'product_type'=>$product_type, 'old_price'=>$old_price, 'price_after_discount'=>$new_price);*/
...
echo "</pre>"; exit;*/
	...
	//public function myqueryfunction($product_label,'limit 0,10'){	
		$this->db->cache_delete_all();
...
ORDER BY pricing_table_metal_options.sku DESC limit 0,10";
//$limit_range
 ...
 WHERE (catalog_product_entity_varchar.attribute_id = 135)";
	...
	public function pknready(){
		...
		$deals_category->load($deals_categoryid);
