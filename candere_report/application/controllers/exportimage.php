	...
	public function index()
	{
		...
		if(!empty($postValue)){
			...
			foreach($pro_sku as $val){
				...
				$sql='SELECT  distinct metal_id,product_id FROM `pricing_table_metal_options` WHERE `sku`="'.trim($val).'"';
				...
				foreach($results_row_img as $key=>$file_img){		
					...
					//echo "<br>";
					//$zip->renameName($path,trim($val).'jpg');
					$zip->addFile($path);
					
					//echo trim($val).'.jpg'; echo "<br>";
					//$zip->renameName($path,trim($val).'_'.$key.'.jpg');
					$zip->renameIndex($path,trim($val).'_'.$key.'.jpg');
