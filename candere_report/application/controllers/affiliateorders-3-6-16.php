	...
	public function download()
	{
		...
		if($user_type=='Buyers') {
		   ...
		   $resultarray 	= $result->result_array();
			...
			$query = "Select subscriber_id,subscriber_email,subscriber_status,gender,affiliate_id,affiliate_term,affiliate_medium,affiliate_content,affiliate_source,subscription_date from newsletter_subscriber where subscriber_status=1 and subscription_date between '$mysql_date_from' AND '$mysql_date_to' " ;
			$result = $this->db->query($query);
			$resultarray 	= $result->result_array();
			for($i=0;$i<count($resultarray);$i++) {
