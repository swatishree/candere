...
class Exportordersdetails extends CI_Controller  {
	...
	public function index()
	{
		...
		,sales_flat_order.mktplace_order_id,sales_flat_order.mktplace_name,sales_order_status.label AS status,sales_flat_order.created_at as 'Created_Date',SFOI.product_id,sales_flat_order.coupon_code,		sales_flat_order.coupon_code2,sales_flat_order.coupon_code3,sales_flat_order.coupon_code4,sales_flat_order.coupon_code5,SFOI.qty_ordered,sales_flat_order.base_shipping_amount,SFOI.base_discount_amount,SFOI.sku as Sku,FROM_UNIXTIME(SFOI.additional_data) as Expected_delivery_date,CONCAT(sales_flat_order.customer_firstname ,' ', sales_flat_order.customer_lastname) as 'Bill_To',SFOP.method,AFST.track_number,

		(SELECT  eav_attribute_option_value.value AS product_type FROM ( ( eav_attribute_option eav_attribute_option INNER JOIN eav_attribute_option_value eav_attribute_option_value ON (eav_attribute_option.option_id = eav_attribute_option_value.option_id)) INNER JOIN catalog_product_entity_int catalog_product_entity_int ON (catalog_product_entity_int.value = eav_attribute_option.option_id)) INNER JOIN catalog_product_entity catalog_product_entity ON (catalog_product_entity.entity_id = catalog_product_entity_int.entity_id) WHERE catalog_product_entity_int.attribute_id = 272 and catalog_product_entity.entity_id=SFOI.product_id) as producttype,
		
		AFST.title,SFOI.base_row_total,SFOI.base_price,
		sales_flat_order.base_grand_total,sales_flat_order.grand_total,sales_flat_order.base_total_paid 	
		from 
		sales_flat_order
		...
		LEFT JOIN sales_flat_shipment_track as AFST ON (AFST.order_id=sales_flat_order.entity_id)
		...
		$sql = "select sales_flat_order.increment_id as 'Order_Id',SFOI.name as 'Product_Name' 
		,sales_flat_order.mktplace_order_id,sales_flat_order.mktplace_name,sales_order_status.label AS status,sales_flat_order.created_at as 'Created_Date',sales_flat_order.coupon_code,		sales_flat_order.coupon_code2,SFOI.product_id,sales_flat_order.coupon_code3,sales_flat_order.coupon_code4,sales_flat_order.coupon_code5,SFOI.qty_ordered,sales_flat_order.base_shipping_amount,SFOI.base_discount_amount,SFOI.sku as Sku,FROM_UNIXTIME(SFOI.additional_data) as Expected_delivery_date,CONCAT(sales_flat_order.customer_firstname ,' ', sales_flat_order.customer_lastname) as 'Bill_To', SFOI.base_price,SFOI.base_row_total, SFOP.method  $inner_track_value, 		
		(SELECT  eav_attribute_option_value.value AS product_type FROM ( ( eav_attribute_option eav_attribute_option INNER JOIN eav_attribute_option_value eav_attribute_option_value ON (eav_attribute_option.option_id = eav_attribute_option_value.option_id)) INNER JOIN catalog_product_entity_int catalog_product_entity_int ON (catalog_product_entity_int.value = eav_attribute_option.option_id)) INNER JOIN catalog_product_entity catalog_product_entity ON (catalog_product_entity.entity_id = catalog_product_entity_int.entity_id) WHERE catalog_product_entity_int.attribute_id = 272 and catalog_product_entity.entity_id=SFOI.product_id) as producttype,
		...
		$count_sql_query = "select sales_flat_order.increment_id  $inner_track_value from sales_flat_order
	...
	//	echo $order_status 	= implode("','",$_GET['order_status']); die;
		...
		foreach($sql->result_object as $row) {
			...
			$method				= ($result[0]->value)? $result[0]->value : $row->method;
			$track_number		= $row->track_number;
			$carrier				= $row->title;
			$mktplace_order_id				= $row->mktplace_order_id;
			$mktplace_name				= $row->mktplace_name;
			$status 			= $row->status;
			...
			$array[] = array(
				...
				'mktplace_order_id'=>$mktplace_order_id,
				'mktplace_name'=>$mktplace_name,
				'Status'=>$status ); 
		}
