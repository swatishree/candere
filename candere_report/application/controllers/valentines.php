...
class Valentines extends CI_Controller  {
	...
	$mgs.='<tr>
		<td align="center" valign="middle" style="padding:7px;">
			<b><span style="font-size:34px;
			color:#820202;">
			Thanks for participating Happy Father’s Day. </span></b>
		</td>
		</tr>';
		...
		$mgs.='</tbody>
			...
			$config = array(
				...
				$this->email->to($email);
				$this->email->subject("Candere Father’s Day Contest");
				$this->email->message($mgs);
				$this->email->send();
			
			

if($name=$this->input->post('name')!="" && $phone=$this->input->post('phone') && $email=$this->input->post('email')){
