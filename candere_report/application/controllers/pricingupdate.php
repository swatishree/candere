...
class Pricingupdate extends CI_Controller  {
	...
	public function insertpricing($upload_data){	 
		...
		$sql =	"LOAD DATA LOCAL INFILE '{$full_path}' INTO TABLE priceupdate FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '' LINES TERMINATED BY '\r\n' (sku,22k,18k,14k,9k,siij_price_22,sigh_price_22,vsgh_price_22,vvsgh_price_22,vvsef_price_22,siij_price_18,sigh_price_18,vsgh_price_18,vvsgh_price_18,vvsef_price_18,siij_price_14,sigh_price_14,vsgh_price_14,vvsgh_price_14,vvsef_price_14,siij_price_9,sigh_price_9,vsgh_price_9,vvsgh_price_9,vvsef_price_9)"; 
		...
		/****************************************************************************/
		//22k  price update
		$sql =	"update pricing_table_metal_options a,  priceupdate b set siij_price=siij_price_22, sigh_price=sigh_price_22,vsgh_price=vsgh_price_22,vvsgh_price=0,vvsef_price=vvsef_price_22 where b.product_id=a.product_id and purity=589";
		$this->db->query($sql); 
		
		$message_arr[] = '<b>Setting siij_price  22 Price : '.$this->db->affected_rows() .' Updates</b><br/>';
		
		//18k  price update
		$sql =	"update pricing_table_metal_options a,  priceupdate b set siij_price=siij_price_18, sigh_price=sigh_price_18,vsgh_price=vsgh_price_18,vvsgh_price=0,vvsef_price=vvsef_price_18 where b.product_id=a.product_id and purity=50";
		$this->db->query($sql); 
		
		$message_arr[] = '<b>Setting siij_price  18k Price : '.$this->db->affected_rows() .' Updates</b><br/>';
		
		//9k  price update
		$sql =	"update pricing_table_metal_options a,  priceupdate b set siij_price=siij_price_9, sigh_price=sigh_price_9,vsgh_price=vsgh_price_9,vvsgh_price=0,vvsef_price=vvsef_price_9 where b.product_id=a.product_id and purity=51";
		$this->db->query($sql); 
		
		$message_arr[] = '<b>Setting siij_price  9k Price : '.$this->db->affected_rows() .' Updates</b><br/>';
		
		//14K  price update
		
		$sql =	"update pricing_table_metal_options a,  priceupdate b set siij_price=siij_price_14, sigh_price=sigh_price_14,vsgh_price=vsgh_price_14,vvsgh_price=0,vvsef_price=vvsef_price_14 where b.product_id=a.product_id and purity=49";
		$this->db->query($sql); 
		
		$message_arr[] = '<b>Setting siij_price  14K Price : '.$this->db->affected_rows() .' Updates</b><br/>';
		...
		$message_arr[] = '<b>Total Execution Time:</b> '.$execution_time.' Mins';
