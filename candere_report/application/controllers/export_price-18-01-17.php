...
class Export_price extends CI_Controller  {
	...
	public function export_product_price()
	{
		...
		$sel_products = "SELECT  `order_id` ,  `customer_name` ,  `sku` ,  `product_name` ,  `expected_delivery_date` ,  `dispatch_date` ,  `unit_price` FROM erp_order WHERE dispatch_date <  '2017-01-18' AND (status !=  '22' && status !=  '23' && status !=  '24' && status !=  '25')";
		...
		//	$array[] = array('Order Id'=>$order_id, 'Customer Name'=>$name, 'Email'=>$email, 'Product Name'=>$product_name, 'SKU'=>$sku, 'Selling Price'=>$selling_price, 'Actual Cost Price'=>$actual_cost_price);
			$array[] = array('Order Id'=>$order_id, 'Customer Name'=>$name, 'Email'=>$email, 'Product Name'=>$product_name, 'SKU'=>$sku, 'Selling Price'=>$selling_price);
		}
