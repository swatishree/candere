...
class Affiliateorders extends CI_Controller  {
	...
	public function download()
	{
		...
		if($user_type=='Buyers') {
 		   ...
 		   $query  = "select Distinct b.email, b.firstname, b.lastname, b.region, b.postcode, a.state, a.base_grand_total, a.created_at ,b.telephone, b.city, b.country_id from sales_flat_order a inner join sales_flat_order_address b on a.entity_id = b.parent_id where b.address_type = 'billing' and  a.created_at between '$mysql_date_from' AND '$mysql_date_to' group by b.email ";
		   ...
		   $resultarray 	= $result->result_array();
			...
			$query = "Select subscriber_email,subscription_date from newsletter_subscriber where subscriber_status=1 and subscription_date between '$mysql_date_from' AND '$mysql_date_to' " ;
			$result = $this->db->query($query);
			$resultarray 	= $result->result_array();
			$this->load->helper('csv'); 
			$this->load->dbutil(); 
